...
def visualize_with_graphviz(nx_graph: nx.DiGraph, epics,
    ...
    for _, data in nx_graph.nodes(data=True):
        issue_sprints = get_sprints(data['issue'])
        if issue_sprints:
    ...
    for node, data in nx_graph.nodes(data=True):
        ...
        issue_sprints = get_sprints(issue)
        if issue_sprints:
            issue_sprints = list(issue_sprints)
            issue_sprints.sort()
            ...
            color = '{:1.3f} {:1.3f} {:1.3f}'.format(hue, 1, 0.8)
    ...
    if group_epics:
        for epic_key, nodes in epics_nodes.items():
            ...
            sub_graph.body.append('label = {}'.format(_get_epic_label(epic)))
            if align_sprints:
                sub_graph.body += [
