...
def create_issue_digraph(epics_stories):
    ...
    if not nx.is_directed_acyclic_graph(graph):
        log.error(f"graph has at least one cycle: {nx.find_cycle(graph)}")
        sys.exit(1)
    return graph
...
def add_longest_ancestor_distance(graph: nx.DiGraph):
    ...
    log.info(f"longest path length: {max(nx.dag_longest_path_length(graph) + 1, 1)}")
    for node in nx.topological_sort(graph):
        max_dist_predecessors = [
            graph.nodes[predecessor]["max_pred_dist"] + 1 for predecessor in graph.predecessors(node)
        ]
        graph.nodes[node]["max_pred_dist"] = max(max_dist_predecessors, default=0)
