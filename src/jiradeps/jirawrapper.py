...
def get_jira_session(user, password, token, server, verify_certificate=True) -> JIRA:
    log.info("initializing JIRA connection...")
    if token:
        jira = JIRA(options={"server": server, "verify": verify_certificate}, token_auth=token, validate=True)
    else:
        jira = JIRA(options={"server": server, "verify": verify_certificate}, auth=(user, password), validate=True)
...
def load_epic_stories(epic: Issue, session: JIRA) -> list[Issue]:
    query = f'type = Story AND "Epic Link" = {epic.key}'
...
def get_sprints(issue: Issue) -> set[str]:
    sprint_field = _get_customfields().get("sprint")
    if not sprint_field or (sprint_str := getattr(issue.fields, sprint_field)) is None:
        return set()

    return {s.group(1) for s in {SPRINT_ID_REGEX.search(sprint_raw) for sprint_raw in sprint_str} if s is not None}
...
def get_blocked_keys(issue: Issue) -> list[str]:
    ...
    return [
        link.outwardIssue.id for link in links if str(link.type) in BLOCKING_RELATIONS and hasattr(link, "outwardIssue")
    ]
